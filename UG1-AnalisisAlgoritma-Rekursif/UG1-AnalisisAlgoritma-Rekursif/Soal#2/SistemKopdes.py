...
def penjualanTertinggi(data, n):
    if n == 1: 
        ...
        sisaTertinggi = penjualanTertinggi(data, n - 1)
        if data[n - 1][1] > sisaTertinggi[1]:
            return data[n - 1]
        else:
            return sisaTertinggi


def diAtasRataRata(penjualan, rataRata):
    jumlah = 0
    for namaBarang, jumlahTerjual in penjualan.items():
        if jumlahTerjual > rataRata:
            jumlah += 1
    return jumlah
